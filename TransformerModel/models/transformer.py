...
# 更新保存路径
SAVE_DIR = OUTPUT_DIR / 'saved_models' / 'transformer'
PLOT_DIR = OUTPUT_DIR / 'plots' / 'transformer'
...
def load_data(train_path, test_path):
    train_df = pd.read_csv(DATA_DIR / train_path)
    test_df = pd.read_csv(DATA_DIR / test_path)
...
def plot_losses(train_losses, val_losses, title):
    ...
    plt.ylabel('MSE Loss')
    plt.legend()
    plt.savefig(out_dir / f'{title}_loss.png')
    plt.close()  # 不显示图像，只保存

    # 将损失数据写入 CSV 文件
    with open(out_dir / f'{title}_loss_data.csv', 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
...
def plot_predictions(input_seq, true_seq, pred_seq, title, scaler_cnt):
    ...
    plt.xlabel('时间步')
    plt.ylabel('租赁数量')
    plt.savefig(out_dir / f'{title}_prediction.png')
    plt.close()  # 不显示图像，只保存
...
def run_experiment(train_loader, val_loader, test_loader, scaler_cnt, features, target, prediction_type, input_window,
    ...
    try:
        for exp in range(start_exp, num_experiments):
            ...
            save_path = SAVE_DIR / f'transformer_model_{prediction_type}_exp{exp + 1}.pth'
            ...
            train_losses, val_losses = train_model(
                ...
                checkpoint_dir=CHECKPOINT_DIR, exp_id=exp + 1
            )
